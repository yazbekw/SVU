...
def get_user_state(user_id):
    ...
    if row:
        ...
        # إضافة حقل used_questions إذا لم يكن موجوداً
        ...
        state = {
            ...
            'quiz_time': None,
            'start_time': None,
            'used_questions': [],  # قائمة بمعرفات الأسئلة المستخدمة في الاختبارات السابقة
        }
...
def get_unanswered_questions(user_id):
    ...
    # استبعاد الأسئلة التي تمت الإجابة عليها أو المستخدمة سابقاً
...
def build_main_menu(user_id=None):
    buttons = [
        ...
        [InlineKeyboardButton("📝 اختبار مخصص", callback_data="custom_quiz")],  # زر جديد
...
async def custom_quiz_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # الحصول على الأسئلة المتاحة (التي لم يُجب عليها ولم تُستخدم سابقاً)
    ...
    # اختيار عدد عشوائي من الأسئلة (بحد أقصى المتاح)
    ...
    state['answers'] = {}
    state['mode'] = 'normal'
    state['quiz_time'] = minutes * 60  # تحويل إلى ثواني
    state['start_time'] = datetime.now().isoformat()
    # تحديث قائمة الأسئلة المستخدمة
